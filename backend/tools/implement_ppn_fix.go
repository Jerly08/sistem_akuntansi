...
func main() {
	...
	fmt.Println("=== IMPLEMENTING PPN ACCOUNTS SEPARATION FIX ===\n")
	...
	// 1. Create PPN Keluaran account (Output VAT - Liability)
	fmt.Println("1. CREATING PPN KELUARAN ACCOUNT:")
	...
	if existingCount == 0 {
		_, err = tx.Exec(`
			...
			VALUES ('2103', 'PPN Keluaran', 'Output VAT - Sales Tax Payable', 'LIABILITY', 'CURRENT_LIABILITY', 
					(SELECT id FROM accounts WHERE code = '2100'), 3, false, true, 0, $1, $1)
		`, time.Now())
		if err != nil {
	...
	// 2. Ensure PPN Masukan is properly set up as Asset
	fmt.Println("\n2. UPDATING PPN MASUKAN ACCOUNTS:")
	
	// Update existing PPN Masukan accounts to ensure correct type
	result, err := tx.Exec(`
		UPDATE accounts 
		SET type = 'ASSET', 
			...
			updated_at = $1
		WHERE code = '2102' AND name LIKE '%PPN%'
	`, time.Now())
	if err != nil {
	...
	fmt.Printf("✅ Updated %d PPN Masukan accounts to ASSET type\n", rowsAffected)

	// 3. Check current journal entries using wrong PPN account for sales
	fmt.Println("\n3. ANALYZING CURRENT JOURNAL ENTRIES:")
	...
	// 4. Fix journal entries to use correct PPN Keluaran account
	fmt.Println("\n4. FIXING JOURNAL ENTRIES:")
	...
	if len(salesToFix) > 0 {
		...
		for _, sale := range salesToFix {
			...
			_, err = tx.Exec(`
				...
				SET account_id = $1
				WHERE journal_id = $2 
				  AND account_id IN (SELECT id FROM accounts WHERE code = '2102') 
				  AND credit_amount > 0
			`, ppnKeluaranID, sale.JournalID)
			if err != nil {
	...
	// Update balances
	_, err = tx.Exec("UPDATE accounts SET balance = $1 WHERE code = '2102'", ppnMasukanBalance)
	if err != nil {
	...
	fmt.Println("\n=== PPN ACCOUNTS SEPARATION FIX COMPLETED ===")
}