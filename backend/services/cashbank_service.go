...
func (s *CashBankService) createDepositJournalEntries(tx *gorm.DB, transaction *models.CashBankTransaction, account *models.CashBank, request DepositRequest, userID uint) error {
	...
	journalEntry := &models.JournalEntry{
		...
		UserID:          userID,
		Status:          models.JournalStatusPosted,
		IsAutoGenerated: len(request.JournalEntries) == 0,
	}
	
	var lines []models.JournalLine
	
	// Use manual journal entries if provided
	if len(request.JournalEntries) > 0 {
		// Use manual entries
		for i, manualEntry := range request.JournalEntries {
			lines = append(lines, models.JournalLine{
				AccountID:    manualEntry.AccountID,
				Description:  manualEntry.Description,
				DebitAmount:  manualEntry.DebitAmount,
				CreditAmount: manualEntry.CreditAmount,
				LineNumber:   i + 1,
			})
		}
	} else {
		// Get or create source account for automatic deposit
		sourceAccount, err := s.getOrCreateDepositSourceAccount(tx)
		if err != nil {
			return fmt.Errorf("failed to get deposit source account: %w", err)
		}
		
		// Use automatic entries
		lines = []models.JournalLine{
			...
			// Credit: Other Income (Revenue increases)
			{
	...
	// Update all affected account balances for manual entries
	if len(request.JournalEntries) > 0 {
		accountBalanceUpdates := make(map[uint]float64)
		for _, entry := range request.JournalEntries {
			// Skip the main cash/bank account as it's already updated above
			if entry.AccountID != account.AccountID {
				accountBalanceUpdates[entry.AccountID] += entry.CreditAmount - entry.DebitAmount
			}
		}
		
		for accountID, balanceChange := range accountBalanceUpdates {
			if err := tx.Model(&models.Account{}).Where("id = ?", accountID).
				Update("balance", gorm.Expr("balance + ?", balanceChange)).Error; err != nil {
				return fmt.Errorf("failed to update account balance for account %d: %w", accountID, err)
			}
		}
	}
...
func (s *CashBankService) createWithdrawalJournalEntries(tx *gorm.DB, transaction *models.CashBankTransaction, account *models.CashBank, request WithdrawalRequest, userID uint) error {
	...
	if len(request.JournalEntries) > 0 {
		...
		// Get or create expense account for automatic withdrawal
		expenseAccount, err := s.getOrCreateWithdrawalExpenseAccount(tx)
		if err != nil {
		...
		// Use automatic entries
		lines = []models.JournalLine{
			// Debit: General Expense (Expense increases)
			{
		...
		// Update expense account balance (Expense account - debit increases balance)
		err = tx.Model(&models.Account{}).Where("id = ?", expenseAccount.ID).
			Update("balance", gorm.Expr("balance + ?", request.Amount)).Error
		if err != nil {
...
type DepositRequest struct {
	...
	Reference      string               `json:"reference"`
	Notes          string               `json:"notes"`
	JournalEntries []ManualJournalEntry `json:"journal_entries"`
}
