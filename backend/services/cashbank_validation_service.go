...
func NewCashBankValidationService(db *gorm.DB, accountingService *CashBankAccountingService) *CashBankValidationService {
	return &CashBankValidationService{
		db:                db,
...
func (s *CashBankValidationService) ValidateAllSync() error {
	...
	if len(discrepancies) > 0 {
		var issueCount = make(map[string]int)
...
// fixBalanceMismatch fixes the balance discrepancy between CashBank and COA
...
// GetSyncStatus returns overall sync status and statistics
...
// ValidateCashBankIntegrity validates individual cash bank integrity
...
// ResetAllCashBankBalances resets all cash bank balances based on their transactions
