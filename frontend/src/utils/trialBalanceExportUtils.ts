...
export function exportTrialBalanceToPDF(
  ...
  // Header section with safe margins
  const leftMargin = 15;
  ...
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text('FINANCIAL SUMMARY', leftMargin, yPosition);
  yPosition += 5;
  ...
  if (includeAccountDetails && data.accounts && data.accounts.length > 0) {
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('ACCOUNT DETAILS', leftMargin, yPosition);
    yPosition += 5;
    ...
    accountTypes.forEach((accountType, typeIndex) => {
      ...
      if (accountsOfType.length > 0) {
        ...
        doc.setFontSize(11);
        doc.setFont('helvetica', 'bold');
        doc.text(`${accountType.toUpperCase()} ACCOUNTS`, leftMargin, yPosition);
        yPosition += 5;
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('GRAND TOTAL', leftMargin, yPosition);
    yPosition += 5;
  ...
  // Footer with safe margins
  const footerY = pageHeight - 15;
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  doc.text('Generated by Sistem Akuntansi', leftMargin, footerY);
  doc.text(`Generated on: ${new Date().toLocaleString('id-ID')}`, pageWidth - rightMargin, footerY, { align: 'right' });
  ...
  if (pageCount > 1) {
    for (let i = 1; i <= pageCount; i++) {
      doc.setPage(i);
      doc.setFontSize(8);
      doc.text(`Page ${i} of ${pageCount}`, pageWidth - rightMargin, footerY - 5, { align: 'right' });
    }
  }
